...
export const SearchComponent = () => {
  ...
  return (
    <>
      ...
      >
        Busque por notícias de seu interesse!
      </h1>
      <Flex
        style={{
      ...
      >
        <Input
          placeholder="Procure aqui um assunto do seu interesse"
        ...
        >
          {!!recomendations &&
            recomendations.map((item, i) => (
              <Button key={i} onClick={() => handleSearchChange(item)}>
                {item}
              </Button>
            ))}
        </Flex>
        <Button type="primary" onClick={() => handleSearchChange(search)}>
          Achar notícias!
        </Button>
      </Flex>
    </>
